...
- **Theme (dark/light)**: `:root` in `style.css` holds the dark palette (`--bg`/`--card`/`--card-hi`/`--text`/`--sub`/`--hairline`); `:root[data-theme="light"]` overrides just those. `app.js` toggles the attribute (`applyTheme()`/`loadTheme()`/`saveTheme()`, `localStorage` key `toolhub.theme`, default `"dark"`) via the ☀️/🌙 button in `renderHome()`. **Only structural colors are themed** — `--green`/`--red`/`--accent` and every per-tool flavor color stay fixed across both themes (same rule as design tokens above). If you hardcode `rgba(255,255,255,…)` or `#fff` for a separator/border on a `var(--card)`/`var(--bg)` surface, use `var(--hairline)` instead or it'll vanish in light mode — this already bit two spots (`counter.css`, `wonglao-randomcard.css`) before `--hairline` existed.
