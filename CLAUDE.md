...
mathquiz       คิดเลขเร็ว sub-game — 3s show / 5s answer flash quiz, typed or multiple-choice (configurable choice count), selectable starting difficulty level (settings.startLevel, 7 levels) that streak then ramps up from (`level = min(startLevel + floor(streak/3), MATH_MAX_LEVEL)`)
