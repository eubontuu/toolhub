...
huay           หวย — full APPS tool (split out of วงเหล้า); loadHuayState() migrates old toolhub.wonglao huayDigits/huayLast once. Roll history (state.history, capped HUAY_HISTORY_MAX=30, newest first) viewable via "ประวัติ" button → showHuayHistoryOverlay, with a clear-all. Ball gradient uses --accent (theme-adaptive, see Theme vars rule)
fortune        ดูดวง — full APPS tool; 2 โหมด ทั้งคู่สุ่มใหม่ได้ไม่จำกัดรอบ (ไม่ซ้ำใบก่อนหน้าทันที): "ประจำวัน" (สุ่มจาก FORTUNE_DATA 100 ใบ — buildFortuneData() ประกอบจาก phrase pools ขนาด 10/9/8/7/11 ที่ coprime กันพอให้ทั้ง 100 id ได้ชุดข้อความไม่ซ้ำ) และ "เฉพาะเรื่อง" (เลือกหัวข้อ การงาน/การเงิน/ความรัก/สุขภาพ จาก FORTUNE_TOPIC_DATA แล้วสุ่มคำทำนายละเอียดของหัวข้อนั้น). หลังเปิดไพ่ พื้นหลังหน้าจอ tint เป็นสีมงคลของใบล่าสุด บน fixed dark base เสมอ (FORTUNE_BG_BASE, ไม่ตามธีม — ดูดวงมี "โทนมืดลึกลับ" เป็น flavor ของตัวเอง, ต่างจาก overlay การ์ดที่ยังใช้ var(--card) ตามธีมปกติ). Orb gradient + mode/topic active state ใช้ --accent (theme-adaptive); lucky-color swatches/tint stay literal (fortune data, not UI flavor). Combined history across both modes (state.history, capped FORTUNE_HISTORY_MAX=30, fortuneHistoryPush) viewable via "📜 ประวัติ" button → showFortuneHistoryOverlay — tap an entry to reopen its full card (fortuneHistoryResolve), clear-all included
...
- **Theme vars**: flavor colors fixed across themes by default; exceptions = `todo.css` (fully theme-var-built), งู's snake body/start-btn, Jump King's player color, หวย's ball gradient (all `--accent`), and sudoku's given/answer/note/conflict colors (`--text`/`--accent`/`--accent` at reduced opacity/`--red` respectively — semantic UI state, not tool "flavor", so they follow theme vars like any other chrome). 2048's tile palette is an intentional fixed-literal exception (the classic game's own palette). ดูดวง's `.fortune-wrap` background is a fixed dark literal regardless of theme (its own mystical-backdrop flavor — see fortune.js/.css comments), while everything drawn on top of it either carries its own themed card background or uses a fixed light color to stay readable against that fixed dark base. New theme → one `THEMES` entry + one `:root[data-theme]` block. Use `var(--hairline)` not hardcoded white/black for borders on themed surfaces.
