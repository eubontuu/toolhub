...
**Chunking (why big values still sync).** Firestore caps a document at 1 MiB, but a single `localStorage` value can be much bigger — ความทรงจำ keeps every book, page and base64 photo under one key, so one document per key silently broke that app's sync past roughly 6 photos. Any value longer than `SYNC_CHUNK_SIZE` (250,000 chars — chosen so even all-Thai text at 3 bytes/char stays under both the 1 MiB document cap and the rules' `size() < 900000` check) is therefore split across `<key>::part<i>` documents, with the main document holding `__TOOLHUB_CHUNKED__:<count>`. The write order matters: count `0` first (meaning "incomplete, don't use"), then every part, then the real count — so an upload cut off halfway makes readers skip that key rather than assemble a corrupt value. `fetchAll()` reassembles on the way back and skips any key whose parts are incomplete. This is entirely generic — the sync layer still knows nothing about any tool's data shape, so a future tool storing something large gets the same treatment for free.

**Failures are visible, not silent.** A push that fails (offline, quota, rules) leaves its key in `toolhub.sync.pending` and records the error; `ToolHubSync.pendingCount()`/`lastError()` surface that as a ⚠️ on the sidebar's "เชื่อมอุปกรณ์" button and a warning block in the panel explaining what's stuck and to retry with "ซิงค์เดี๋ยวนี้". A `localStorage`-full error while pulling is reported the same way.

**Real-time, but draft-safe.** Whenever linked, `startLiveListener(code)` keeps a Firestore `onSnapshot` listener open on `syncCodes/{code}/data`. Incoming data is written to `localStorage` immediately and unconditionally (never lost), but the UI only refreshes automatically when `isUserBusy()` is false — no focused `<input>`/`<textarea>`/`contenteditable`, and no open `.reveal-overlay.show`. If the owner is mid-edit, a tappable pill (`showLiveUpdateBanner()`, "🔄 มีข้อมูลใหม่จากอีกเครื่อง — แตะเพื่ออัปเดต") appears instead of yanking the screen out from under them; tapping it calls `render()`. Snapshots carrying `metadata.hasPendingWrites` are skipped — those are the device's own writes echoing back before the server confirms them, and reacting to them would just be reacting to yourself. This still isn't real collaborative editing: there's no per-field merge, a whole key is last-write-wins, so two devices editing the exact same thing at the same moment still means one overwrites the other — real-time just means "immediately," not "merged." The manual "ซิงค์เดี๋ยวนี้" button (`ToolHubSync.pullNow()`) and the boot-time pull remain as fallbacks if the listener never connects (offline at boot, listener error). That panel (`showSyncPanel()`, a `reveal-overlay`) is reached via a "เชื่อมอุปกรณ์" button in the sidebar footer (`app.js`'s `openSidebarOverlay()`) — create a new code (`ToolHubSync.createAndLink()`), join an existing one (`ToolHubSync.link(code)` — pulls first, so cloud data for a key wins over this device's local copy for that key; keys only present locally get pushed up to fill gaps), copy the current code, force a sync, or unlink (`ToolHubSync.unlink()` — behind a native `confirm()` in the panel; wipes every `toolhub.*` key on **this device** via `syncableKeys()` and tears down the live listener, so data is strictly scoped to whichever code is currently linked and never lingers to leak into a different code linked afterward — cloud data for the old code is untouched and reachable again by re-linking the same code).
