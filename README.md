...
**History:** every reveal in either mode pushes onto one combined `state.history` (`fortuneHistoryPush`, newest first, capped at `FORTUNE_HISTORY_MAX` = 30) — a `📜 ประวัติ` button (with a live count badge) pinned to the bottom of the screen via `margin-top: auto` (`.fortune-history-fab`, kept separate from the mode-tabs row on purpose) opens `showFortuneHistoryOverlay`, listing every past reveal regardless of which mode it came from. `fortuneHistoryResolve(h)` turns a `{type: "daily", id}` or `{type: "topic", topic, id}` history entry back into its display info and full card HTML — tapping an entry reopens that exact card via `showFortuneOverlay` (read-only, no "next" button), and a "ล้างประวัติ" button clears the list.
...
| `toolhub.huay` | `{ digits, last, history: [{value, digits, time}] }` | หวย — `history` is newest-first, capped at `HUAY_HISTORY_MAX` (30), viewable/clearable via the "ประวัติ" button (`showHuayHistoryOverlay`); `loadHuayState()` migrates a pre-split `huayDigits`/`huayLast` out of `toolhub.wonglao` on first read if `toolhub.huay` doesn't exist yet |
| `toolhub.fortune` | `{ mode: "daily"\|"topic", dailyLastId: number\|null, topicLast: {topic, id}\|null, history: [{type, id, topic?, time}] }` | ดูดวง — `dailyLastId`/`topicLast` are just the last-shown card per mode (for the "ผลล่าสุด" preview + no-immediate-repeat check), not a daily lock — both modes are redrawable anytime; `history` is the combined newest-first log across both modes, capped at 30 |
